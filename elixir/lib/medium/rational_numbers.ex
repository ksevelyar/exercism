defmodule RationalNumbers do
  ...
  def subtract({n1, d1}, {n2, d2}), do: {n1 * d2 - n2 * d1, d1 * d2} |> reduce()
  ...
  def pow_rational({n, d}, pow) when pow >= 0, do: {n ** pow, d ** pow} |> reduce()
  def pow_rational({n, d}, pow), do: {d ** -pow, n ** -pow} |> reduce()
